...
@Component({
  ...
  styles: [`
    ...
    .eyebrow { font-size: 0.78rem; font-weight: 700; letter-spacing: 0.14em; text-transform: uppercase; color: var(--gold); margin: 0 0 1rem; }
    ...
    .section-label { font-size: 0.78rem; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; color: var(--text-tertiary); margin: 0 0 1.25rem; }

    /* Featured */
    .featured-section { margin-bottom: 3.5rem; }
    .featured-card { display: grid; grid-template-columns: 400px 1fr; border-radius: 20px; overflow: hidden; background: var(--surface); border: 1px solid var(--border); box-shadow: 0 8px 32px rgba(33,27,20,0.08); }
    ...
    .post-card { background: var(--surface); border: 1px solid var(--border); border-radius: 16px; overflow: hidden; transition: transform 150ms ease, box-shadow 150ms ease; &:hover { transform: translateY(-3px); box-shadow: 0 12px 32px rgba(33,27,20,0.1); } }
    ...
    /* Common */
    .cat-badge { display: inline-block; font-size: 0.68rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; padding: 3px 9px; border-radius: 999px; background: var(--brand-light); color: var(--brand); }
    ...
    /* Newsletter */
    .newsletter { margin-top: 3.5rem; background: linear-gradient(135deg, var(--brand) 0%, var(--brand-hover) 100%); border-radius: 20px; padding: clamp(2.5rem, 5vw, 4rem) 2rem; }
    ...
    .nl-input { flex: 1; border: none; border-radius: 999px; padding: 12px 20px; font-family: inherit; font-size: 0.95rem; background: rgba(255,255,255,0.95); &:focus { outline: 2px solid rgba(255,255,255,0.6); outline-offset: 2px; } }
    .nl-btn { background: var(--text-primary); color: var(--bg-primary); border: none; border-radius: 999px; padding: 12px 22px; font-family: inherit; font-weight: 700; font-size: 0.9rem; cursor: pointer; white-space: nowrap; transition: background 140ms ease; &:hover { background: #000; } }

    @media (max-width: 800px) { .featured-card { grid-template-columns: 1fr; } .featured-vis { min-height: 180px; } }
    @media (max-width: 480px) { .nl-form { flex-direction: column; } .nl-input, .nl-btn { border-radius: 12px; } }
  `]
})
